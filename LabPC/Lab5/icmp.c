...
void ping(int sockfd, struct in_addr ip_addr, int count)
{
	...
	printf("msg from %s: icmp_seq=%hu ttl=%u time=%.1f\n",
                ip_s, ntohs(ricmp_hdr->un.echo.sequence), rip_hdr->ttl,
                time_elapsed);
}
}
...
void traceroute(int sockfd, struct in_addr ip_addr)
{
	...
	while (!dst_reached) {
		...
		icmp_hdr->type = ICMP_ECHO;
		icmp_hdr->un.echo.id = 0;
		icmp_hdr->un.echo.sequence = htons(hop_idx);
		icmp_hdr->checksum = 0;
		...
		dst_reached = ricmp_hdr->type != ICMP_TIMXCEED;
		struct in_addr addr;
		addr.s_addr = rip_hdr->saddr;
		printf("%2d %s %.03f ms\n", hop_idx, inet_ntoa(addr), time_elapsed);
